...
export let baselineTemplateData = [
    {
        TaskID: 1,
...
export let resourceAllocationData = [
    {
        TaskID: 1,
        TaskName: 'Project initiation',
        StartDate: new Date('03/29/2025'),
...
export let resourceAllocationResources = [
...
const childPerParent = 20;
const blockSize = childPerParent + 1;
const totalParents = Math.floor(100000 / blockSize);

let taskId = 1;

for (let i = 1; i <= totalParents; i++) {
    ...
    for (let j = 1; j <= 20; j++) {
        ...
        if (j === 1) {
            seq1Start = currentTaskId;
        }

        if (j === 8) {
            seq2Start = currentTaskId;
            predecessor = seq1Start + 'FS';
        }

        if (j === 15) {
            seq3Start = currentTaskId;
            predecessor = seq2Start + 'FS';
        }

        if (j === 20) {
            predecessor = seq3Start + 'FS';
        }

        virtualData.push({
            TaskID: currentTaskId,
            TaskName: `Project ${i} - Task ${j}`,
            StartDate: new Date(2025, 0, j),
            Duration: (j % 3) + 2,
            Progress: 30,
...
export let infiniteTimelineScrollData = [
    {
        TaskID: 1,
